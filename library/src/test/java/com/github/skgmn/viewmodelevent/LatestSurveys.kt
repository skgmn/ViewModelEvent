...
class LatestSurveys {
    ...
    fun delayedOnStopStart() = runBlockingTest {
        ...
        assertTrue(answer1.isCancelled)
        assertTrue(answer2.isCancelled)
    ...
    fun latterRespondEarlierThanFormer() = runBlockingTest {
        ...
        val answer1 = async { activity.viewModel.pendingSurvey.ask(signal1).single() }
        val answer2 = async { activity.viewModel.pendingSurvey.ask(signal2).single() }

        assertFalse(answer1.isCancelled)
        assertFalse(answer2.isCancelled)
    ...
    class TestActivity : AppCompatActivity() {
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            answer(viewModel.normalSurvey, DeliveryMode.LATEST) {
                it.toString()
            }
            answer(viewModel.pendingSurvey, DeliveryMode.LATEST) {
                it.first().toString()
            }
            answer(viewModel.recreateSurvey, DeliveryMode.LATEST) {
